...
<script>
  export default {
    ...
    methods: {
        ...
        getlistData(){
          this.$get('teachers/'+this.id+'/classes/'+this.form.lesson+'/getclassreport').then((res) => {
            if(res.code === 0){
              ...
              if(res.data.lesson){
                for(var i=0,len = res.data.itemList.length ; i< len;i++){
                  ...
                  res.data.itemList[i].exam = res.data.itemList[i].answer.toString()
                }
        ...
        submit(){
          ...
          if(!this.data.lesson){
            for(var i=0; i <this.list.length; i++ ){
              item={"itemId":this.list[i].itemId,"choose":this.list[i].exam,"memo":this.form.memo}
                this.exam.push(item)
            }            
          }
          if(this.data.lesson){
            for(var i=0; i <this.list.length; i++ ){
              if(!this.list[i].exam && this.list[i].exam==='' ){
                flag = 1
                return
              }else{
                item={"itemId":this.list[i].itemId,"choose":this.list[i].exam,"memo":this.form.memo,"id":this.list[i].id}
                this.exam.push(item)
              }
            ...
            if(flag == 2) {
            this.$post("teachers/"+this.id+"/classes/"+this.form.lesson+"/classreports",{
